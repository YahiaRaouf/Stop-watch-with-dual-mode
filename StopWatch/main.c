...
unsigned char lapMin = 0;
unsigned char lapHrs = 0;
...
void display_7Segment(unsigned char num, unsigned char lap,
		unsigned char segment) {
	PORTA = (PORTA & 0x00) | (1 << segment);
	PORTC = (PORTC & 0x00) | (num & 0x0F) | (lap << 4);
}
...
int main() {
	...
	while (1) {
		...
		lap();
		display_7Segment(sec % 10, lapSec % 10, 0);
		_delay_ms(2);
		display_7Segment(sec / 10, lapSec / 10, 1);
		_delay_ms(2);
		display_7Segment(min % 10, lapMin % 10, 2);
		_delay_ms(2);
		display_7Segment(min / 10, lapMin / 10, 3);
		_delay_ms(2);
		display_7Segment(hrs % 10, lapHrs % 10, 4);
		_delay_ms(2);
		display_7Segment(hrs / 10, lapHrs / 10, 5);
		_delay_ms(2);
